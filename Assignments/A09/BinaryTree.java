...
public class BinaryTree implements Serializable {
	...
	private int numLeaves(Node node) {
		...
		if (node.left != null) {
			num += numLeaves(node.left);
		}
		if (node.right != null) {
			num += numLeaves(node.right);
		}
		return num;
	}
	...
	private int numNodesWithOneChild(Node node) {
		...
		if (node.left == null && node.right != null) {
			num += 1 + numNodesWithOneChild(node.right);
		}
		if (node.left != null && node.right == null) {
			num += 1 + numNodesWithOneChild(node.left);
		}
		if (node.left != null && node.right != null) {
			num += numNodesWithOneChild(node.left);
			num += numNodesWithOneChild(node.right);
		}
		return num;
	}
